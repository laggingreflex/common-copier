...
module.exports = class CommonCopier extends CLI {
  ...
  link = command({
    ...
    handler: async function link() {
      ...
      console.log(`this.fileLimit:`, this.fileLimit);
      console.log(`this.memory:`, this.memory);
      this.memory = 1
      await this.$save({ key: 'memory' });
      ...
      if (this.remember) {
        ...
        //     ...this.remember,
        //     [this.projectDir]: [
        //       ...(this.remember[this.projectDir] || []),
        //       this.commonDir,
        //     ],
        //   }
        // })
      } else {
        console.log('Use --remember to remember this linkage for future');
      }
      console.log('Done!');
    }
  });
  ...
  async readdir(path, { size = 0, start = +new Date } = {}) {
    ...
    size += files.length;
    if (size > this.fileLimit) throw new _.UserError(`Files exceeded --file-limit=${this.fileLimit}`);
  ...
  /** @type {boolean} Remember --project-dir & --common-dir. Pass twice (-r -r) to add to memory rather than replacing it when passing new set of dirs */
  remember = option({ type: 'count', alias: ['r'], description: `Remember --project-dir & --common-dir. Pass twice (-r -r) to add to memory rather than replacing it when passing new set of dirs` });
  memory = option({ type: 'object' });
